...
app.get('/pinball_machines', async(req, res) => {
  try {
    ...
    SELECT pinball_machines.id AS pinball_id, pinball_machines.name, pinball_machines.year_manufactured, manufacturers.manufacturer, pinball_machines.multiball, pinball_machines.owner_id FROM pinball_machines
    JOIN manufacturers
    ON manufacturers.id = pinball_machines.manufacturer_id
    ORDER BY pinball_machines.id asc
    `);
